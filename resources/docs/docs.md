...
The `home-routes` handler in the `v2ex-clojure.routes.home` namespace
...
The `home-page` function will in turn call the `v2ex-clojure.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `v2ex-clojure.handler` namespace:
...
Request middleware functions are located under the `v2ex-clojure.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `v2ex-clojure.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `v2ex-clojure.db.core` in some other namespace.
